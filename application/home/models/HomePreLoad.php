...
class HomePreLoad extends Zend_Controller_Plugin_Abstract
{
  /**
   * 全局可用的Zend_Acl对象
   *
  ...
  function __construct()
  {
    if (FALSE === Zend_Registry::isRegistered('zendAcl'))
    {
      ...
      // 需要控制的资源 ....
	  $Acl->add(new Zend_Acl_Resource('home_welcome'));
      // 默认权限规则
      $Acl->allow(array('guest', 'member', 'admin'));
      // 设置控制规则 ....
      $Acl->deny('guest', array('home_welcome'));
  ...
  public function preDispatch($_request)
  {
    ...
    $action = $_request->action;
  	// Layout初始化
  	Zend_Layout::startMvc(array('layoutPath' => '../../application/layouts/', 'layout' => 'home'));
  	// ajax请求自动关闭布局和渲染
	...
	else
	{
		...
		<div class="sub-nav span-13 last"></div>
		');
	}
    // 当前访问者身份
