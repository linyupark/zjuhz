<?php
require_once(dirname(__FILE__) . "/Zjuhz_userconn.php");
...
function findAll() {
	 ...
	 */
	$fields = array('uid','username','password','realName','sex','birthday','year','college','major','hometown_c','location_c');
	...
	$rscount = mysql_query("SELECT count(*) AS cnt FROM `tbl_user` $where"); 
	...
	$query_recordset = "SELECT uid,username,password,realName,sex,birthday,year,college,major,hometown_c,location_c FROM `tbl_user` $where $order";
...
function insert() {
	...
	$query_insert = sprintf("INSERT INTO `tbl_user` (username,password,realName,sex,birthday,year,college,major,hometown_c,location_c) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)" ,			GetSQLValueString($_REQUEST["username"], "text"), # 
			GetSQLValueString($_REQUEST["password"], "text"), # 
			GetSQLValueString($_REQUEST["realName"], "text"), # 
			GetSQLValueString($_REQUEST["sex"], "text"), # 
			GetSQLValueString($_REQUEST["birthday"], "text"), # 
			GetSQLValueString($_REQUEST["year"], "int"), # 
			GetSQLValueString($_REQUEST["college"], "text"), # 
			GetSQLValueString($_REQUEST["major"], "text"), # 
			GetSQLValueString($_REQUEST["hometown_c"], "text"), # 
			GetSQLValueString($_REQUEST["location_c"], "text")# 
	);
		...
		$toret = array(
			"data" => array(
				array(
					...
					"hometown_c" => $_REQUEST["hometown_c"], # 
					"location_c" => $_REQUEST["location_c"]# 
				)
			), 
			"metadata" => array()
		);
...
function update() {
	...
	$query_recordset = sprintf("SELECT * FROM `tbl_user` WHERE uid = %s",
		GetSQLValueString($_REQUEST["uid"], "int")
	);
	$recordset = mysql_query($query_recordset, $conn);
	...
	if ($num_rows > 0) {
		...
		$query_update = sprintf("UPDATE `tbl_user` SET username = %s,password = %s,realName = %s,sex = %s,birthday = %s,year = %s,college = %s,major = %s,hometown_c = %s,location_c = %s WHERE uid = %s", 
			...
			GetSQLValueString($_REQUEST["hometown_c"], "text"), 
			GetSQLValueString($_REQUEST["location_c"], "text"), 
			GetSQLValueString($row_recordset["uid"], "int")
		);
...
function delete() {
	...
	if ($num_rows > 0) {
		$row_recordset = mysql_fetch_assoc($recordset);
		$query_delete = sprintf("DELETE FROM `tbl_user` WHERE uid = %s", 
			GetSQLValueString($row_recordset["uid"], "int")
		);
